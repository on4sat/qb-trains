...
RegisterNetEvent("AskForTrain", function()
	TriggerServerEvent("RequestTrain")
end)

RegisterNetEvent("AskForTrainConfirmed", function()
	...
	if metroDist < countryDist then
		if metroDist < Config.RequestDistance then
			QBCore.Functions.Progressbar("AskForTrain", "Accessing public transport..",(Config.WaitingTime * 1000), false, false, {
				...
				TriggerServerEvent("qb-trains:ticketpayment")
				QBCore.Functions.Notify("Train shall arrive shortly..", "success")
				Citizen.Wait(Config.WaitingTime * 1000)
				StartMetro(closestMetro)
				QBCore.Functions.Notify("Train will depart from the station", "success")
			end, function() -- Cancel
				QBCore.Functions.Notify("You are not near the train station!", "error")			
			end)
		else
			QBCore.Functions.Notify("You are not near the train station!", "error")
		end
...
RegisterNetEvent('Trains:ToggleTainsBlip', function()
   ...
   for _, item in pairs(Config.TrainStations) do
        ...
        if not showTrainStations then
			...
			AddTextComponentString("Railway Station")
		...
		if IsControlJustReleased(1,  38) then
			...
			if IsThisModelATrain( GetEntityModel(targetVehicle) ) and not intrain then
				...
				if IsPedInAnyTrain(playerped) then
					intrain = true
					QBCore.Functions.Notify("You have boarded the train..")
				else
					QBCore.Functions.Notify("the train is full","error")
...
function StartMetro(coords)
	...
	SetNetworkIdCanMigrate(id, false)
	QBCore.Functions.Notify("Train will leave in 30 seconds [E] to enter")
	Citizen.Wait((Config.TimeToBoard * 1000) - Config.TimeWarning * 1000)
	QBCore.Functions.Notify("Train will leave in 5 seconds")		
...
function StartCountry(coords)
	...
	local id = NetworkGetNetworkIdFromEntity(trailer)
	SetNetworkIdCanMigrate(id, false)
	QBCore.Functions.Notify("Train will leave in 30 seconds")
	Citizen.Wait((Config.TimeToBoard * 1000) - Config.TimeWarning * 1000)
	QBCore.Functions.Notify("Train will leave in 5 seconds")
	Citizen.Wait(Config.TimeWarning * 1000)
	stopsrunning2 = true
...
RegisterNetEvent('spawn-ped',function(coords)
	local hash = `a_m_m_prolhost_01`
